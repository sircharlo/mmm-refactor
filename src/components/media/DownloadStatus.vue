<template>
  <q-btn :icon="Object.values(downloadProgress).filter(item => item.error).length === 0 ?
    'mdi-cloud-check-variant' : 'mdi-cloud'" :loading="Object.values(downloadProgress).filter(item => item.loaded).length > 0" flat push round
    size="md">
    ...
    <q-popup-proxy>
      <div :class="'rounded-borders bg-grey-' + $q.dark.isActive ? '2' : '9'" style="width: 400px; height: 300px;">
        <q-list :class="'q-px-md non-selectable bg-grey-' + $q.dark.isActive ? '2' : '9'" dense separator>
          <q-item style="align-items: center;" v-if="Object.values(downloadProgress).length === 0">
          ...
          <template :key="statusObject.status" v-for="statusObject in statusConfig">
            <q-item-label class="q-pt-md text-weight-bold text-uppercase" overline
              v-if="hasStatus(downloadProgress, statusObject.status)">
              {{ $t(statusObject.label) }}
            </q-item-label>
            <q-item :key="url" style="align-items: center;"
              v-for="(item, url) in filteredDownloads(downloadProgress, statusObject.status)">
              <q-item-label caption lines="1">
                {{ basename(url as string) }}
              </q-item-label>
              <q-space />
              <q-item-section avatar class="q-pr-none" style="align-items: end;">
                <q-icon :color="statusColor(statusObject.status)" :name="statusObject.icon" v-if="statusObject.icon" />
                <q-circular-progress :thickness="0.3" :value="progressValue(item)" color="primary"
                  size="18px" style="margin-right: 3px;" v-else-if="showProgress(item)" />
              </q-item-section>
            </q-item>
          </template>
        </q-list>
